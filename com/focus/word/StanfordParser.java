...
public class StanfordParser
{
    ...
    protected static final Pattern escaped_char_pattern = Pattern
        .compile("\\\\/");
    protected static final Pattern double_quote_lable_pattern = Pattern
        .compile("[`'][`']");
    protected static final Pattern bracket_label_pattern = Pattern
        .compile("-...-");
    ...
    public static void initialize() throws Exception
    {
        ...
        String modelFile = "/Users/mvundela/eclipse-workspace/FocusWordFinder/config/englishPCFG.ser.gz";
    ...
    protected static void updateTreeLabels(Tree root, Tree tree,
        MutableInteger offset, MutableInteger leafIndex)
    {
        if (tree.isLeaf())
        {
            leafIndex.value++;
        ...
        if (double_quote_lable_pattern.matcher(labelValue).matches()
            && length > 1)
        {
            offset.value--;
     ...
     *         offsets in a white-space depleted version of the input string
     *         back into offsets in the input string.
     */
    protected static List<RangeMap> createMapping(String sentence)
    {
        ...
        while (whitespace_matcher.find())
        {
            ...
            else
            {
                ...
                delta_rmap = new DeltaRangeMap(delta_rmap.end, 0,
                    delta_rmap.delta + length);
            }
        }
     ...
     * 
     * @param tree
     *            the Tree whose begin and end extents should be mapped.
     * @param mapping
     *            the list of RangeMap objects which defines the mapping.
     */
    ...
    // String str = null;
    // if (begin < 0 || begin > sentence.length() || end < begin || end >
    // sentence.length()) {
    // str = "error";
    // } else {
